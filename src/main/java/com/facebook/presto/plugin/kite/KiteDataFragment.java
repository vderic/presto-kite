...
public class KiteDataFragment
{
    private static final JsonCodec<KiteDataFragment> MEMORY_DATA_FRAGMENT_CODEC = jsonCodec(KiteDataFragment.class);

    private final HostAddress hostAddress;
    private final long rows;

    @JsonCreator
    public KiteDataFragment(
    ...
    public static KiteDataFragment fromSlice(Slice fragment)
    {
        return MEMORY_DATA_FRAGMENT_CODEC.fromJson(fragment.getBytes());
    }

    public static KiteDataFragment merge(KiteDataFragment a, KiteDataFragment b)
    {
        checkArgument(a.getHostAddress().equals(b.getHostAddress()), "Can not merge fragments from different hosts");
        return new KiteDataFragment(a.getHostAddress(), a.getRows() + b.getRows());
    }
}
