...
import static com.facebook.presto.plugin.kite.KiteErrorCode.MEMORY_LIMIT_EXCEEDED;
import static com.facebook.presto.plugin.kite.KiteErrorCode.MISSING_DATA;
import static java.lang.String.format;

@ThreadSafe
public class KitePagesStore
{
    ...
    @Inject
    public KitePagesStore(KiteConfig config)
    {
    ...
    public synchronized void add(Long tableId, Page page)
    {
        ...
        if (maxBytes < newSize) {
            throw new PrestoException(MEMORY_LIMIT_EXCEEDED, format("Kite limit [%d] for memory connector exceeded", maxBytes));
        }
    ...
    public synchronized void cleanUp(Set<Long> activeTableIds)
    {
        ...
        // That can lead to a situation when KitePagesStore already knows about a newer second table on some worker
        // but cleanUp is triggered by insert from older first table, which KiteTableHandle was created before
