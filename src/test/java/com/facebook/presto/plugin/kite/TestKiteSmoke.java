        ...
        extends AbstractTestQueryFramework
{
    ...
    protected QueryRunner createQueryRunner()
            throws Exception
    {
        return KiteQueryRunner.createQueryRunner();
    }

    @Test
    public void testCreateAndDropTable()
    {
        int tablesBeforeCreate = listKiteTables().size();
        assertUpdate("CREATE TABLE test AS SELECT * FROM tpch.tiny.nation", "SELECT count(*) FROM nation");
        assertEquals(listKiteTables().size(), tablesBeforeCreate + 1);

        assertUpdate("DROP TABLE test");
        assertEquals(listKiteTables().size(), tablesBeforeCreate);
    }

    // it has to be RuntimeException as FailureInfo$FailureException is private
    @Test(expectedExceptions = RuntimeException.class, expectedExceptionsMessageRegExp = "line 1:1: Destination table 'kite.default.nation' already exists")
    ...
    {
        assertQueryFails("DROP SCHEMA schema1", "line 1:1: Schema 'kite.schema1' does not exist");
        assertUpdate("CREATE SCHEMA schema1");
        assertQueryFails("CREATE SCHEMA schema1", "line 1:1: Schema 'kite.schema1' already exists");
    ...
    public void testCreateTableAndViewInNotExistSchema()
    {
        ...
        int tablesAfterCreate = listKiteTables().size();
    ...
    public void testRenameTable()
    {
        ...
        assertQueryFails("ALTER TABLE test_table_to_be_renamed RENAME TO kite.test_schema_not_exist.test_table_renamed", "Schema test_schema_not_exist not found");
    ...
    public void testViews()
    {
        ...
        assertQueryFails("DROP VIEW test_view", "line 1:1: View 'kite.default.test_view' does not exist");
    }

    private List<QualifiedObjectName> listKiteTables()
    {
        return getQueryRunner().listTables(getSession(), "kite", "default");
    }
