...
public class TestKitePagesStore
{
    ...
    private KitePagesStore pagesStore;
    private KitePageSinkProvider pageSinkProvider;

    @BeforeMethod
    public void setUp()
    {
        pagesStore = new KitePagesStore(new KiteConfig().setMaxDataPerNode(new DataSize(1, DataSize.Unit.MEGABYTE)));
        pageSinkProvider = new KitePageSinkProvider(pagesStore, HostAddress.fromString("localhost:8080"));
    }
    ...
    @Test(expectedExceptions = PrestoException.class)
    public void testKiteLimitExceeded()
    {
        createTable(0L, 0L);
    ...
    private void insertToTable(long tableId, Page page, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                KiteTransactionHandle.INSTANCE,
                SESSION,
                createKiteInsertTableHandle(tableId, activeTableIds),
    ...
    private void createTable(long tableId, Long... activeTableIds)
    {
        ConnectorPageSink pageSink = pageSinkProvider.createPageSink(
                KiteTransactionHandle.INSTANCE,
                SESSION,
                createKiteOutputTableHandle(tableId, activeTableIds),
                PageSinkContext.defaultContext());
        pageSink.finish();
    }

    private static ConnectorOutputTableHandle createKiteOutputTableHandle(long tableId, Long... activeTableIds)
    {
        return new KiteOutputTableHandle(
                new KiteTableHandle(
                        "test",
                        "schema",
    ...
    private static ConnectorInsertTableHandle createKiteInsertTableHandle(long tableId, Long[] activeTableIds)
    {
        return new KiteInsertTableHandle(
                new KiteTableHandle(
                        "test",
                        "schema",
                        format("table_%d", tableId),
